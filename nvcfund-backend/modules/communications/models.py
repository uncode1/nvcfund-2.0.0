...
class CommunicationLog(db.Model):
    ...
    recipient_user_id = db.Column(db.String(255), db.ForeignKey('users.id'), nullable=True)
...
class CommunicationPreference(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id = db.Column(db.String(255), db.ForeignKey('users.id'), nullable=False, unique=True)
...
class CommunicationAttachment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    communication_log_id = db.Column(db.Integer, db.ForeignKey('communication_logs.id'), nullable=False)
