...
class ChatService:
    ...
    def _find_available_agent(self, agent_type: str, user_id: int, session_local) -> Optional[Agent]:
        ...
        try:
            ...
            # Find agent
            agent = session_local.query(Agent).filter_by(
...
class AgentService:
    ...
    def get_available_agents(self, user_role: str = 'standard') -> List[Dict[str, Any]]:
        ...
        session_local = SessionLocal()
        try:
            return session_local.query(Agent).filter(
                Agent.is_available == True,
                Agent.min_user_role <= user_role  # Role hierarchy check
            ).all()
        finally:
            session_local.close()
