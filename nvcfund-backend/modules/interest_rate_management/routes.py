...
def main_dashboard():
    ...
    try:
        ...
        if current_user.role == UserRole.TREASURY_OFFICER.value:
            ...
            return redirect(url_for('interest_rate_management.executive_dashboard'))
        else:
            return redirect(url_for('interest_rate_management.admin_dashboard'))
            
    except Exception as e:
        error_logger.log_error(
            message=f"Error in interest rate main dashboard: {str(e)}",
...
def treasury_officer_dashboard():
    ...
    try:
        ...
        error_logger.log_error(
            message=f"Error in treasury officer dashboard: {str(e)}",
...
def treasury_officer_dashboard_page():
    ...
    try:
        ...
        error_logger.log_error("TREASURY_DASHBOARD_ERROR", str(e), {"user_id": current_user.id})
...
def asset_liability_manager_dashboard_page():
    ...
    try:
        ...
        error_logger.log_error("ALM_DASHBOARD_ERROR", str(e), {"user_id": current_user.id})
