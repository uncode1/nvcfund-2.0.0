...
from modules.binance_integration.services import BinanceIntegrationService
...
def exchange_dashboard():
    ...
    try:
        ...
        external_rates = []
        try:
            if binance_service.is_authenticated():
                external_rates = binance_service.get_ticker_prices()
        ...
        return render_template('exchange/dashboard.html',
                             ...
                             binance_connected=binance_service.is_authenticated())
...
def external_exchange():
    ...
    try:
        ...
        if not binance_service.is_authenticated():
        ...
        trading_pairs = binance_service.get_trading_pairs()
        
        # Get ticker prices
        ticker_prices = binance_service.get_ticker_prices()
...
def get_exchange_quote():
    ...
    try:
        ...
        if exchange_type == 'internal':
            ...
            quote = service.get_external_quote(from_currency, to_currency, amount, binance_service)
        else:
...
def execute_exchange():
    ...
    try:
        ...
        if exchange_type == 'internal':
            ...
            if not binance_service.is_authenticated():
                return jsonify({'error': 'External exchange not connected'}), 400
            result = service.execute_external_exchange(current_user.id, data, binance_service)
        else:
            return jsonify({'error': 'Invalid exchange type'}), 400
