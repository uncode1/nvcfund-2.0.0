...
import sys
sys.path.insert(0, '$PROJECT_ROOT')
from nvcfund-backend.modules.auth.services import AuthService
...
import sys
sys.path.insert(0, '$PROJECT_ROOT')
from nvcfund-backend.modules.banking.services import BankingService
...
if command -v pytest >/dev/null 2>&1; then
    ...
    pytest "$PROJECT_ROOT/nvcfund-backend/tests/" -v --tb=short >> "$TEST_RESULTS_FILE" 2>&1 || {
...
REPORT_FILE="$LOG_DIR/monthly_update_report_$TIMESTAMP.txt"
{
    ...
    echo "Security Status: $(grep 'Security Status:' "$LOG_DIR/update_status_$TIMESTAMP.txt" | cut -d' ' -f3)"
    echo ""
...
if grep -q "READY" "$LOG_DIR/analysis_status_$TIMESTAMP.txt"; then
